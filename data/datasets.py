...
class MultiFramesDataset(data.Dataset):
    ...
    def __init__(self, opts):
        ...
        for dataset, task in self.dataset_task_list: # dataset:DAVIS       task:WCT/antimonocromatismo
            list_filename = os.path.join(opts.list_dir, "%s_%s.txt" %(dataset, self.mode))
            print("[%s] Read %s (Task %s)" %(self.__class__.__name__, list_filename, task))

            with open(list_filename) as f:
                videos = [line.rstrip() for line in f.readlines()] # Get the phase_dataset's subdirs' name

            for video in videos:
                self.task_videos.append([task, os.path.join(dataset, video)]) # os.path.join(dataset, video): DAVIS/bear
    ...
    def __getitem__(self, index):
        ...
        for t in range(T, T + self.opts.sample_frames):
            frame_i.append( util.read_img(os.path.join(input_dir, video, "%05d.jpg" %t) ) )
            frame_p.append( util.read_img(os.path.join(process_dir, video, "%05d.jpg" %t) ) )
